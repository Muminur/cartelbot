...
**Fixed Python `/client/start` 400 error**: Root cause: encryption scheme mismatch - Next.js sends plain token, Python tried Fernet decryption. Removed decrypt_token() call from client_manager.py, renamed parameter to `token`, added format validation (50-150 chars). Test passed: HTTP 200 with `{"success":true}`. Python service working. **Added debug logging**: Frontend logs request body, backend logs received Discord user info. Fixed local variable assignment bug in fallback validation path.
