...
export class BinanceClient {
  ...
  async createOCOOrder(
    ...
    // Try new OCO API first, then fall back to legacy API, then individual orders
    // This handles Binance -2010 inconsistencies with the new endpoint
    try {
      // Attempt 1: New OCO endpoint with above/below terminology
      // For SELL orders:
      // - aboveType: LIMIT_MAKER (take profit above current price)
      // - belowType: STOP_LOSS_LIMIT (stop loss below current price)
      console.log(`[OCO] ${symbol} - Using new API (/api/v3/orderList/oco)`);
      ...
      this.updateOrderRateLimit();
      console.log(`[OCO] ${symbol} - New API succeeded`);
      return result;
    } catch (newApiError) {
      // Only fall back for -2010 (insufficient balance) errors
      if (!(newApiError instanceof BinanceAPIError && newApiError.binanceCode === -2010)) {
        throw newApiError;
      }
  ...
  /**
   * Create OCO order using the legacy /api/v3/order/oco endpoint.
   * This endpoint is deprecated but still functional and uses the old parameter names.
   * Used as fallback when the new /api/v3/orderList/oco endpoint returns -2010.
   */
  async createLegacyOCOOrder(
    symbol: string,
    quantity: number,
    price: number,
    stopPrice: number,
    ...
    // eslint-disable-next-line no-console
    console.log("OCO Order Parameters (Legacy API):", {
      symbol,
      quantity: formattedQuantity,
    ...
    // Legacy endpoint uses: price, stopPrice, stopLimitPrice, stopLimitTimeInForce
    // (no aboveType/belowType/abovePrice/belowStopPrice/belowPrice/belowTimeInForce)
