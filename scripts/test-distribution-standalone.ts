/**
 * Standalone test for target distribution validation
 * No environment dependencies - pure logic testing
 */

// Inline the validation function for standalone testing
function isValidDistribution(distribution: number[] | undefined | null): boolean {
...
testCases.forEach((testCase, index) => {
  ...
  if (result === testCase.expected) {
    ...
    failed++;
    const failMsg = `${status} [${index + 1}/${testCases.length}] ${testCase.name}\n   Expected: ${testCase.expected}, Got: ${result}\n   Input: ${JSON.stringify(testCase.input)}`;
